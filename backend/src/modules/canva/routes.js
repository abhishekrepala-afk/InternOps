...
async function routes(fastify) {
  ...
  fastify.get('/auth/url', {
    schema: { tags: ['Canva'], description: 'Get Canva OAuth authorization URL' },
  }, async (req) => {
    const url = service.getAuthUrl();
    if (!url) {
      return { success: false, error: 'Canva integration not configured. Set CANVA_CLIENT_ID and CANVA_CLIENT_SECRET in .env' };
    }
  ...
  fastify.get('/auth/callback', {
    ...
    if (error || !code) {
      return reply.redirect(`${process.env.APP_URL || 'http://localhost:5173'}/admin/canva-templates?error=${error || 'no_code'}`);
    }
    ...
    try {
      ...
      return reply.redirect(`${process.env.APP_URL || 'http://localhost:5173'}/admin/canva-templates?success=true`);
    } catch (err) {
      return reply.redirect(`${process.env.APP_URL || 'http://localhost:5173'}/admin/canva-templates?error=${err.message}`);
    }
  });
  ...
  fastify.post('/import/:designId', {
    schema: { tags: ['Canva'], description: 'Import Canva design as certificate template' },
  }, async (req, reply) => {
    const template = await service.importDesignAsTemplate(req.params.designId, req.user.id);
  ...
  fastify.post('/export/:certificateId', {
    schema: { tags: ['Canva'], description: 'Export certificate as Canva design' },
  }, async (req, reply) => {
    const design = await service.exportCertificateToCanva(req.params.certificateId);
